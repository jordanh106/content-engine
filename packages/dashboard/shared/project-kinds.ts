...
export const PROJECT_KIND_REGISTRY: Record<ProjectKind, ProjectKindDefinition> = {
  ...
  viral_replication: {
    ...
    icon: "Repeat2",
    group: "showcase",
    generationMode: { type: "orchestrator", endpoint: "generate-viral-replication" },
    refsMinimum: 0,
    steps: STANDARD_STEPS_NO_REFS,
    expectedOutputs: [
      { label: "Hook variant", kind: "text", count: 1, blurb: "Scored hook + opening line" },
      { label: "Rebuilt scene stills", kind: "image", count: 3, blurb: "Three frames that echo the source structure" },
      { label: "Rebuilt reel motion", kind: "video", count: 1, blurb: "5-7s clip seeded by the strongest still" },
    ],
    briefSections: VIRAL_REPLICATION_SECTIONS,
    generateCtaLabel: "Generate replication",
    generateBlurb: "Borrows the structure of a viral reference and recasts it with your brand.",
  },
  ad_variants: {
    kind: "ad_variants",
    ...
    icon: "GitBranch",
    group: "showcase",
    generationMode: { type: "orchestrator", endpoint: "generate-ad-variants" },
    refsMinimum: 0,
    steps: STANDARD_STEPS_NO_REFS,
    expectedOutputs: [
      { label: "Variant fan-out", kind: "image", count: 6, blurb: "Six on-brand variants across the axes you specify" },
    ],
    briefSections: AD_VARIANTS_SECTIONS,
    generateCtaLabel: "Generate variants",
    generateBlurb: "Fans out six on-brand ad variants. Scored, ranked, ready to ship.",
  },
  product_360: {
    kind: "product_360",
    label: "Product 360",
    icon: "Wand2",
    group: "showcase",
    generationMode: { type: "orchestrator", endpoint: "generate-product-360" },
    refsMinimum: 1,
    steps: STANDARD_STEPS_WITH_REFS,
    expectedOutputs: [
      { label: "Turntable frames", kind: "image", count: 8, blurb: "Eight rotation frames around your product" },
    ],
    briefSections: PRODUCT_360_SECTIONS,
    generateCtaLabel: "Generate 360",
    generateBlurb: "Builds an eight-frame product turntable seeded by your reference image.",
  },
  press_kit: {
    kind: "press_kit",
    label: "Press kit",
    icon: "Image",
    group: "showcase",
    generationMode: { type: "orchestrator", endpoint: "generate-press-kit" },
    refsMinimum: 0,
    steps: STANDARD_STEPS_NO_REFS,
    expectedOutputs: [
      { label: "Square hero (1:1)", kind: "image", count: 1, blurb: "Instagram / LinkedIn primary" },
      { label: "Portrait hero (4:5)", kind: "image", count: 1, blurb: "Feed-optimised vertical" },
      { label: "Wide hero (16:9)", kind: "image", count: 1, blurb: "Press release / web" },
      { label: "Story hero (9:16)", kind: "image", count: 1, blurb: "Reels / TikTok / stories" },
    ],
    briefSections: PRESS_KIT_SECTIONS,
    generateCtaLabel: "Generate press kit",
    generateBlurb: "Generates four editorial hero stills across the formats publishers ask for.",
  },
  generic: {
    kind: "generic",
